#!/usr/bin/env bash
# Baseline config, usage_limit_fraction=0.5, disk spill enabled, single-session.
set -euo pipefail
...
CONFIG="$BENCH_REPO/configs/memory_usage_limit/usage_limit_0P5.yaml"

OUT="$RUN_DIR/sweep_usage_limit_0P5"
